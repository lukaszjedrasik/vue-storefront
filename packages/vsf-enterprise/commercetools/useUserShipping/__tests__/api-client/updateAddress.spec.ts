...
const updateAddressVariables = (addressId?) => {
  ...
  if (addressId !== undefined) {
    data.actions.unshift({ setDefaultShippingAddress: { addressId } });
  }
...
const updateAddressResponse: any = (address, defaultId) => ({
  data: {
    user: {
      version: 2,
      addresses: [ address ],
      defaultShippingAddressId: defaultId
    }
  }
});

describe('[ct-shipping] API - updateAddress', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  it('updates address and returns array of addresses', async () => {
    ...
    expect(result).toEqual({
      ...
      defaultShippingAddressId: params.user.defaultShippingAddressId,
      version: 2
    });
  });
